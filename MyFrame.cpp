...
MyFrame::MyFrame(const wxString& title) : 
	...
	auto m_staticText1 = new wxStaticText( this, wxID_ANY, wxT("Ïóòü ê èñõîäíîé äèðåêòîðèè"), wxDefaultPosition, wxDefaultSize, 0 );
	...
	auto m_staticText11 = new wxStaticText( this, wxID_ANY, wxT("Óñòàíîâèòå ïîðîãîâîå çíà÷åíèå"), wxDefaultPosition, wxDefaultSize, 0 );
	...
	wxString m_ModeChoices[] = { wxT("ïî êîëè÷åñòâó"),wxT("ïî îáúåìó") };
	...
	m_Start = new wxButton( this, wxID_ANY, wxT("Íà÷àòü"), wxDefaultPosition, wxDefaultSize, 0 );
	m_prefBox->Add( m_Start, 1, wxALL, 5 );

	m_ShowEmpty = new wxButton( this, wxID_ANY, wxT("Îòîáðàçèòü ïóñòûå"), wxDefaultPosition, wxDefaultSize, 0 );
	m_prefBox->Add( m_ShowEmpty, 1, wxALL, 5 );

	m_Save = new wxButton( this, wxID_ANY, wxT("Ñîõðàíèòü"), wxDefaultPosition, wxDefaultSize, 0 );
	...
	auto m_staticText4 = new wxStaticText( this, wxID_ANY, wxT("Ñîâïàäàþùèå äèðåêòîðèè"), wxDefaultPosition, wxDefaultSize, 0 );
	...
	m_Info = new wxButton( this, wxID_INFO, wxT("Îòîáðàçèòü"), wxDefaultPosition, wxDefaultSize, 0 );
	...
	auto m_staticText5 = new wxStaticText( this, wxID_ANY,wxT("Óíèêàëüíûå äèðåêòîðèè"), wxDefaultPosition, wxDefaultSize, 0 );
...
void MyFrame::Binds()
	...
	m_dirPicker1->Bind(wxEVT_DIRPICKER_CHANGED,[&](wxFileDirPickerEvent &evt)
		{
		//åñëè èçìåíèëàñü äèðåêòîðèÿ î÷èùàåì
		//óñòàíàâëèâàåì ñòàíäàðòíîå ïîðîãîâîå çíà÷åíèå
		Clear();
	...
	m_Start->Bind(wxEVT_BUTTON, [&](wxCommandEvent&)
		{
		//îñíîâíîé îáðàáîò÷êèê
		Clear();
		auto root = m_dirPicker1->GetPath();
		//ïîëó÷àåì êàðòó äèðåêòîðèé-ôàéëîâ
		Fill(root);
		//óäàëÿåì êîðåíü
		m_dirs.erase(root);
        Groups2 data;
		std::unordered_set<wxString> dub;
		//ïåðåáîð âñåõ äèðåêòîðèé
		for(auto it = m_dirs.cbegin();it!=m_dirs.cend();it++)
			{
			//åñëè äèðåêòîðèÿ ïóñòàÿ - ïðîäîëæàåì
			if(it->second.empty())
				{
				m_emptyDirs.insert(it->first);
				continue;
				}
			//ñðàâíèâàåì ñî âñåìè ïîñëåäóþùèìè
			for(auto j = std::next(it);j!=m_dirs.cend();j++)
				{
				bool is_eq;
				std::size_t percent;
				//ñðàâíèâàåì â çàâèñèìîñòè îò ðåæèìà
				...
				if(is_eq)
					{
					//åñëè äèðåêòîðèè ýêâèâàëåíòíû
					//òî äîáàâëÿåì ïóòè â íàáîð äóáëèêàòîâ
					//è ñîõðàíÿåì ïðîöåíò ñîîòâåòñòâèÿ
					dub.insert(it->first);
					dub.insert(j->first);
...
//çàïîëíåíèå êàðòû , êëþ÷ - äèðåêòîðèÿ, çíà÷åíèå - âåêòîð ôàéëîâ
...
std::pair<bool,std::size_t> MyFrame::CountCompare(
	...
	//îïðåäåëÿåì ìàêñèìàëüíî äîïóñòèìîå êîëè÷åñòâî ðàçëè÷íûõ ôàéëîâ
	std::size_t MaxDiff = maxsize-std::ceil(maxsize*Limit/100);
	//åñëè êîëè÷åñòâî ôàéëîâ â ïàïêàõ îòëè÷àåòñÿ íà áîëüøåå - âîçâðàò
	if(maxsize-std::min(a.size(),b.size())>MaxDiff)
		return std::make_pair(false,0);
	//íóæíî äëÿ îñîáîãî ó÷åòà äóáëåé
	...
	for(const auto &f1 : a)
		...
		for(const auto &f2 : b)
			...
			//åñëè ðàçìåð ñîâïàäàåò
			//è ðàíåå ýòîò ôàéë íå áûë íàéäåí
			//è îí èäåíòè÷åí ïðîâåðÿåìîìó
			//äîáàâëÿåì â ïîìå÷åííûå è îòìå÷àåì ñîîòâåòñòâèå
